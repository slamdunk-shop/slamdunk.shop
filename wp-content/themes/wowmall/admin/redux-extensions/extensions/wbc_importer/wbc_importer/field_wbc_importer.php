...
if ( ! class_exists( 'ReduxFramework_wbc_importer' ) ) {
	...
	class ReduxFramework_wbc_importer {
		...
		function __construct( $field = array(), $value = '', $parent ) {
			...
			add_filter( 'current_theme_supports-demo_skins', array( $this, 'current_theme_supports' ), 10, 3 );
		}

		public function current_theme_supports( $return, $args, $feature ) {
			if( empty( $feature ) || empty( $feature[0] ) ) {
				return false;
			}
			foreach ( $args as $arg ) {
				if( in_array( $arg, $feature[0] ) ) {
					return true;
				}
			}
			return false;
		...
		public function render() {
			...
			if ( ! empty( $this->field['wbc_demo_imports'] ) ) {
				...
				foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
					...
					if ( 'Bikes' === $imports['directory'] && ! current_theme_supports( 'demo_skins', 'Bikes' ) ) {
						continue;
					}
					...
					if ( ! array_key_exists( 'imported', $imports ) ) {
						...
						$imported       = false;
						$import_message = esc_html__( 'Import Demo', 'wowmall' );
					}
					else {
						$imported       = true;
						$extra_class    = 'active imported';
						$import_message = esc_html__( 'Demo Imported', 'wowmall' );
					}

					if ( ! $activated ) {
						$extra_class    = 'not-activated';
						$import_message = esc_html__( 'Activate to Import', 'wowmall' );
					}
					...
					if ( ! $activated ) {
						echo '<div class="wbc-importer-buttons"><span class="button-primary importer-button import-demo-data">' . esc_html__( 'Activate to Import', 'wowmall' ) . '</span></div>';
					}
					else {
						if ( false == $imported ) {
							echo '<div class="wbc-importer-buttons"><span class="spinner">' . esc_html__( 'Please Wait...', 'wowmall' ) . '</span><span class="button-primary importer-button import-demo-data">' . esc_html__( 'Import Demo', 'wowmall' ) . '</span></div>';
						}
						else {
							echo '<div class="wbc-importer-buttons button-secondary importer-button">' . esc_html__( 'Imported', 'wowmall' ) . '</div>';
							echo '<span class="spinner">' . esc_html__( 'Please Wait...', 'wowmall' ) . '</span>';
							echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">' . esc_html__( 'Re-Import', 'wowmall' ) . '</div>';
						}
					}
					echo '</div>';
					echo '</div>';
				...
				echo "<h5>" . esc_html__( 'No Demo Data Provided', 'wowmall' ) . "</h5>";
			}
		...
		public function enqueue() {
			...
			if ( $activated ) {
				...
				wp_enqueue_script( 'redux-field-wbc-importer-js', $this->extension_url . 'field_wbc_importer.js', array( 'jquery' ), time(), true );
			}

			wp_enqueue_style( 'redux-field-wbc-importer-css', $this->extension_url . 'field_wbc_importer.css', time(), true );

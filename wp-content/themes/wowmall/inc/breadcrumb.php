...
/**
 * Wowmall_Breadcrumb class.
 *
 * @class Wowmall_Breadcrumb
 */
class Wowmall_Breadcrumb {
	...
	public function generate() {
		...
		if ( ! ( is_post_type_archive() && get_option( 'page_on_front' ) === wc_get_page_id( 'shop' ) ) || is_paged() ) {
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'wowmall' ) );
	}
	...
	private function add_crumbs_product_tag() {
		...
		$this->prepend_shop_page();
		$this->add_crumb( sprintf( esc_html__( 'Products tagged &ldquo;%s&rdquo;', 'wowmall' ), $current_term->name ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Posts tagged &ldquo;%s&rdquo;', 'wowmall' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Author: %s', 'wowmall' ), $userdata->display_name ) );
	}
	...
	private function endpoint_trail() {
		...
		if ( class_exists( 'WC' ) && is_wc_endpoint_url() && ( $endpoint = WC()->query->get_current_endpoint() ) && ( $endpoint_title = WC()->query->get_endpoint_title( $endpoint ) ) ) {
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'wowmall' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( esc_html__( 'Page %d', 'wowmall' ), get_query_var( 'paged' ) ) );
		}
	}
}

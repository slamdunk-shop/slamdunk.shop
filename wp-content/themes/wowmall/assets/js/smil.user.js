...
function smile( animating ) {
	...
	ns2proc[ svgns ]       = true; //&& !impl.hasFeature("org.w3c.svg.animation", "1.0");
	ns2proc[ smilanimns ]  = true;
	ns2proc[ smil2ns ]     = true;
	ns2proc[ smil21ns ]    = true;
	ns2proc[ smil3ns ]     = true;
	ns2proc[ timesheetns ] = true;
...
function Animator( anim, target, index ) {
	...
	this.anim.getStartTime   = function () {
		return (me.iterBegin - timeZero) / 1000;
	};
...
try {
	...
	window.addEventListener( "fakesmile_init", initSMIL, false );
} catch ( exc ) {
}
